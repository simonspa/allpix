...
AllPixTimepix3Digitizer::AllPixTimepix3Digitizer(G4String modName, G4String hitsColName, G4String digitColName) 
...
AllPixTimepix3Digitizer::~AllPixTimepix3Digitizer(){

}
void AllPixTimepix3Digitizer::SetDetectorDigitInputs(G4double thl){
...
G4double AllPixTimepix3Digitizer::MyErf(G4double x){
...
vector<G4double>  AllPixTimepix3Digitizer::ComputeDriftTimeFullField(G4double x, G4double y, G4double z)
{
...
void AllPixTimepix3Digitizer::ComputeElectricField(G4double /*x*/, G4double /*y*/, G4double z){

		Efield1D(z);
}


G4double AllPixTimepix3Digitizer::GetElectricFieldNorm(G4double /*x*/, G4double /*y*/, G4double /*z*/){

	return TMath::Sqrt(electricFieldX*electricFieldX +electricFieldY*electricFieldY +electricFieldZ*electricFieldZ );
}

G4double  AllPixTimepix3Digitizer::MobilityElectron(G4double /*x*/, G4double /*y*/, G4double /*z*/){
...
G4double AllPixTimepix3Digitizer::MobilityHole(G4double x, G4double y, G4double z){
...
G4double AllPixTimepix3Digitizer::ComputeDriftTimeUniformField(AllPixTrackerHit * /*a_hit*/)
{
...
G4double AllPixTimepix3Digitizer::ComputeDiffusionRMS(G4double tDrift)
{
	return TMath::Sqrt(2.*Default_Electron_D*tDrift);

}

G4double AllPixTimepix3Digitizer::ApplyTrapping(G4double tDrift, G4double Energy)
{
...
G4double AllPixTimepix3Digitizer::IntegrateGaussian(G4double xhit,G4double yhit,G4double Sigma, G4double x1, G4double x2, G4double y1, G4double y2, G4double Energy )
{
...
void AllPixTimepix3Digitizer::Efield1D(G4double z){
 ...
 */
void AllPixTimepix3Digitizer::Digitize(){

	// Create the digits collection
	m_digitsCollection = new AllPixTimepix3DigitsCollection("AllPixTimepix3Digitizer", collectionName[0] );
...
//		if( (fabs(xpos) >= pitchX/2.-3*sigma || fabs(ypos) >=pitchY/2.-3*sigma) ){
	...
	for( ; pCItr != pixelsContent.end() ; pCItr++)
	{
		...
		if((*pCItr).second > threshold)
		{
			...
			AllPixTimepix3Digit * digit = new AllPixTimepix3Digit;
...
vector<G4double>  AllPixTimepix3Digitizer::RKF5Integration(G4double x, G4double y, G4double z,G4double dt)
{
...
G4double  AllPixTimepix3Digitizer::SetDt(G4double dt, G4double ErreurMoy)
{
...
G4int AllPixTimepix3Digitizer::EnergyToTOT(G4double Energy, G4double threshold)
{
	...
	TOT = TMath::FloorNint((SaturationEnergy/elec-threshold)/(dEdt*Lv1Unit));
	}
	else{
	TOT = TMath::FloorNint((Energy-threshold)/(dEdt*Lv1Unit));
	}
...
//G4int AllPixTimepix3Digitizer::EnergyToTOTFEI4(G4double Energy, G4double threshold)
//{
...
//______________________________________________________________________________
G4double AllPixTimepix3Digitizer::SlimEdgeEffect(G4int nX,G4double xpos,G4double eHit)
{
...
G4bool AllPixTimepix3Digitizer::isSlimEdge(G4int nX, G4int /*nY*/)
{
